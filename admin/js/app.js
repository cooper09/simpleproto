...
controllers.controller('DataCtrl', function ($scope, $http) {
   ...
   $scope.response = null;
    ...
    $http.jsonp('http://sonyainc.net/polyads/php/test_poly_data_2.php?format=jsonp&callback=JSON_CALLBACK').success(function(data, status) {
        alert('data: ' +  data[0].campaign );
        $scope.workData = data;
    ...
    $http.jsonp('http://sonyainc.net/services/todo/get_personal_data.php?format=jsonp&callback=JSON_CALLBACK').success(function(data, status) {
     ...
     $http.jsonp('http://sonyainc.net/services/todo/get_groceries_data.php?format=jsonp&callback=JSON_CALLBACK').success(function(data, status) {
      ...
      $scope.putWorkData = function($scope,taskname) {
        $scope.newTask = '';
         ...
         $scope.showRefresh = 'Testing testing 1..2..3..'; 
      }
